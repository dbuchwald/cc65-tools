...
This target depends on ROM file, so when invoked (`make test`), it will check if the file exists. If it doesn't, it will be created using other recipes. Unlike `all` target it doesn't stop here. When built is completed successfully, it will invoke two commands:
...
You can also see new `make` internal variable: `$<` represents first entry in dependency list.

Last but not least, there is `clean` target:
...
This one has no dependencies, and when invoked will be executed each time. It will remove all the files generated by the build process.

## Actual assembly code

Source code used in this example is very simple, and you should be familiar with it from Ben's videos. Just to recap:
...
We have two main sections here, one at address 0x8000, where the actual code resides, and another one at address 0xFFFC containing reset vector (address of the code to be executed after 6502 reset sequence). 

The code itself is pretty simple: first it sends 0xFF to 0x6002 address, representing VIA DDRB register, and resulting in setting all PORT B pins as outputs. Then it sets the value of output PORT B to 0x50, saving this value to VIA PORTB register at address 0x6000.

Main loop is very simple, it will just keep rotating the value sent to PORT B causing the LEDs connected to PORT B to blink.

## Building example

There are two ways to build this example. One is to use command line from `cc65-tools-main/tutorial` folder:

`docker run --rm -v/Users/dawid/Downloads/cc65-tools-main/tutorial:/mnt/project dawidbuchwald/cc65-tools-make -C 01_blink_vasm test`

Note that the syntax is slightly different compared to the [previously used](README.md) - and this is intentional. Invoking `make` with `-C FOLDER_NAME` parameter will cause `make` to go into this folder and execute the desired target (`test` in this case).

````
➜  tutorial docker run --rm -v/Users/dawid/Downloads/cc65-tools-main/tutorial:/mnt/project dawidbuchwald/cc65-tools-make -C 01_blink_vasm test
make: Entering directory '/mnt/project/01_blink_vasm'
...
hexdump -C ../build/01_blink_vasm.bin
...
00008000
md5sum ../build/01_blink_vasm.bin
762bda266c36142c0da217d034f813c4  ../build/01_blink_vasm.bin
make: Leaving directory '/mnt/project/01_blink_vasm'
````

You can also use Visual Studio Code and run "Terminal"->"Run Build Task..."->"Test Single Project" and choosing "01_blink_vasm". You just have to make sure that you selected `tutorial` folder when opening project in Visual Studio Code.

## Summary

So, this is all you need to know about `makefile` syntax for now. Read the files discussed above again to make sure you understand all the recipes in them, and when you feel confident it makes sense, move on to [next section](02_blink.md).
